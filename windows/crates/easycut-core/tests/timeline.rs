...
fn timeline_ops_match_selftest() {
    ...
    assert!(p.tracks[0].clips.len() == 2 && p.clip(right).unwrap().source_in == 4.0, "분할: 4초에서 둘로");

    p.delete_clips(&HashSet::from([id]), true);
    assert!(close(p.duration(), 6.0) && p.tracks[0].clips[0].start == 0.0, "리플 삭제: 뒤 클립이 당겨짐");
    ...
    assert!(close(p.duration(), 5.0) && p.tracks[0].clips.len() == 2, "구간 리플 삭제 1~2초");
    let second = p.tracks[0].clips[1].clone();
    assert!(close(second.start, 1.0) && close(second.source_in, 6.0), "구간 삭제 후 원본 위치 유지");

    p.trim_end(second.id, 3.0, Some(10.0));
    assert!(close(p.duration(), 3.0), "끝 트림");

    let b = p.insert(&a, 0, 1.5, 5.0);
    assert!(p.clip(b).unwrap().start >= 1.5 && p.tracks[0].clips.len() == 3, "겹침 해결: 겹친 클립 밀어내기");

    let mut q = Project::default();
    q.captions = vec![Caption::new(0.0, 2.0, "a"), Caption::new(3.0, 5.0, "b"), Caption::new(6.0, 8.0, "c")];
...
fn abc_project() -> Project {
    ...
    v.captions = vec![Caption::new(0.0, 2.0, "A"), Caption::new(2.2, 5.0, "B"), Caption::new(5.5, 9.0, "C")];
    v
}
...
fn caption_move_to_front() {
    ...
    assert!(close(v.tracks[0].clips[0].source_in, 5.5), "영상도 C 구간(5.5초~)부터 시작");
}
...
fn transcript_project() -> Project {
    let words: Vec<Word> =
        (0..12).map(|i| Word::new(format!("단어{i}"), i as f64 * 0.7 + 0.2, i as f64 * 0.7 + 0.7)).collect();
...
fn delete_words_cuts_video() {
    ...
    assert!(((before - p.duration()) - removed).abs() < 0.01, "단어 3개 삭제 → {removed:.2}초 잘림");
    let tw2 = p.timeline_words();
    assert_eq!(tw2.len(), tw.len() - 3);
    assert!(!tw2.iter().any(|w| del.contains(&w.id())), "삭제된 단어가 대본에서 사라짐");
}
...
fn filler_words() {
    ...
    let tw = p.timeline_words();
    let picked: Vec<&str> = tw.iter().filter(|w| ids.contains(&w.id())).map(|w| w.word.text.as_str()).collect();
...
fn audio_silence_detection() {
    ...
    assert!(close(sil[1].start, 3.6) && close(sil[1].end, 4.3), "파일 끝은 여유 없이 끝까지");
    ...
    let r = p.audio_silence_ranges(&map, &silence::SilenceSettings { threshold: th, min_silence: 0.6, padding: 0.1 });
    assert_eq!(r.len(), 2);
    p.tracks[0].muted = true;
    assert!(p.audio_silence_ranges(&map, &silence::SilenceSettings::default()).is_empty(), "음소거 트랙 제외");
}

#[test]
fn whisper_chunks_cover_all() {
    // 16kHz 10초, 3~6초 조각
    let s: Vec<i16> = (0..160_000).map(|i| if (i / 16000) % 4 == 3 { 0 } else { ((i % 50) * 200) as i16 }).collect();
...
fn whisper_full_json() {
    ...
    assert!(close(w[0].start, 0.25) && close(w[0].end, 1.05), "DTW 시간 - 0.15");
    ...
    assert_eq!(whisper::parse_progress("whisper_print_progress_callback: progress =  45%"), Some(0.45));
}
