...
final class RecordController: ObservableObject {
    enum Phase: Equatable { case idle, countdown(Int), recording, saving }
    ...
    func begin() async {
        ...
        let opts = ScreenRecorder.Options(
            display: display, excludeApp: excludeApp,
        ...
        do {
            ...
            timer = Timer.scheduledTimer(withTimeInterval: 0.25, repeats: true) { [weak self] _ in
                Task { @MainActor in if let self { self.elapsed = Date().timeIntervalSince(self.startedAt) } }
            }
        } catch {
            finishUI()
    ...
    func stop() async {
        guard phase == .recording, let rec = recorder else { return }
        phase = .saving
        timer?.invalidate(); timer = nil
        do {
            let r = try await rec.stop()
            finishUI()
            await store?.importRecording(screen: r.screen, camera: r.camera)
        } catch {
            finishUI()
    ...
    private func showPanel(on display: SCDisplay) {
        ...
        let size = NSSize(width: 250, height: hasCam ? 230 : 64)
...
extension EditorStore {
    ...
    func importRecording(screen: URL, camera: URL?) async {
        do {
            ...
            apply { p in
                ...
                let t = wasEmpty ? 0 : max(p.trackEnd(0), p.trackEnd(1))
